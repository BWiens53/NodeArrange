...
class NA_X_AlignNodes(Operator):
    '''Align the selected nodes on x axis'''
    bl_idname = "node.na_align_node_x"
    bl_label = "Align Nodes X"
    bl_options = {'REGISTER', 'UNDO'}
    margin: IntProperty(name='Margin', default=0, description='The amount of space between nodes')
    ...
    def execute(self, context):
        ...
        active_loc = None
        if not selection:
            raise Exception("No nodes have been selected")
...
class NA_Y_AlignNodes(Operator):
    '''Align the selected nodes on Y axis'''
    bl_idname = "node.na_align_node_y"
    bl_label = "Align Nodes Y"
    bl_options = {'REGISTER', 'UNDO'}
    margin: IntProperty(name='Margin', default=0, description='The amount of space between nodes')
...
class NA_PT_NodePanel(Panel):
    ...
    def draw(self, context):
        if context.active_node is not None:
            ...
            row = layout.row()
            row.operator('node.na_align_node_x', text="Align Vertically")
            row.operator('node.na_align_node_y', text="Align Horizontally")
